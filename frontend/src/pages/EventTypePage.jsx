...
import { EVENT_META, EVENT_TYPES, SERVICE_PAGE } from "@/servicesContent";
...
export default function EventTypePage() {
  ...
  if (!eventType) return <Navigate to="/services/event" replace />;

  const p = SERVICE_PAGE[lang];
  const bullets = eventType.bullets[lang];

  const scrollToBooking = () =>
    document.getElementById("rezervo")?.scrollIntoView({ behavior: "smooth" });

  return (
    <div className="bg-ink text-white font-body min-h-screen" data-testid="event-type-page">
      ...
      <header className="fixed top-0 inset-x-0 z-40 px-4 md:px-8 pt-4">
        <div className="glass-strong rounded-full max-w-7xl mx-auto flex items-center justify-between pl-6 pr-3 py-3">
          <Link to="/" data-testid="event-logo-link" className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <ArrowLeft size={14} />
              <span className="hidden sm:inline">{EVENT_META.backEvents[lang]}</span>
            </Link>
          </div>
        </div>
      </header>
      ...
      <section className="relative min-h-[62vh] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 pt-44 pb-14 w-full">
          <motion.span
            ...
            className="text-xs tracking-[0.3em] uppercase text-gold/80"
          >
            {lang === "sq" ? "Siguria e Eventeve" : "Event Security"}
          </motion.span>
          <motion.h1
      ...
      <section className="relative py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4" data-testid="event-benefits-list">
            {bullets.map((b, i) => (
              <motion.div
                key={i}
      ...
      <section id="rezervo" className="relative py-16 md:py-24" data-testid="rezervo-section">
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <p className="text-xs tracking-[0.3em] uppercase text-gold mb-4">{eventType.name[lang]}</p>
          ...
          >
            <LeadForm
              endpoint="/booking"
              buildExtra={() => ({ service: `Event Security — ${eventType.name.en}` })}
              title={p.formTitle}
              text={p.formText}
              submitLabel={p.cta}
              testPrefix="booking"
